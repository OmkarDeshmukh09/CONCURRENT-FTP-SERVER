...
int main()
{
    ...
    char* massage = "Jay ganesh from Server\n";
    ...
    // Step 3 : Mark the Socket as aactive socket
    
    iRet = listen (ServerScoket, 11);               // 11 specify the total number of clients can handle

    if (iRet == -1)
    {
        printf("Unable to convert socket in the listen mode \n");
        return -1;
    }

    printf("Server is live at poart %d \n",Port);

 
//  Step 4 : Accept new Socket Request.

    ClientSocketfd = accept(ServerScoket, NULL, NULL);

    if (ClientSocketfd == -1)
    {
        printf("Unablr to accept request from Client !");
        return -1;
    }
    
    printf("Request accepted ny the Server");
    
//  Step 5 : Send the massage to client

    write(ClientSocketfd, massage, strlen(massage));
